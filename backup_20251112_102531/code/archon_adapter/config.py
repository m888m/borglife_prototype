...
class ArchonConfig(BaseModel):
    ...
    def from_env(cls) -> 'ArchonConfig':
        ...
        return cls(
            archon_server_url=os.getenv('ARCHON_SERVER_URL', 'http://archon-server:8181'),
            archon_mcp_url=os.getenv('ARCHON_MCP_URL', 'http://archon-mcp:8051'),
            archon_agents_url=os.getenv('ARCHON_AGENTS_URL', 'http://archon-agents:8052'),
            supabase_url=os.getenv('SUPABASE_URL', ''),
            supabase_service_key=os.getenv('SUPABASE_SERVICE_KEY', ''),
            enable_docker_mcp_organs=os.getenv('ENABLE_DOCKER_MCP_ORGANS', 'true').lower() == 'true',
            ...
            circuit_breaker_reset_timeout=int(os.getenv('CIRCUIT_BREAKER_RESET_TIMEOUT', '300')),
...
class BorglifeConfig(BaseModel):
    ...
    def from_env(cls) -> 'BorglifeConfig':
        ...
        return cls(
            ...
            min_feedbacks_for_reputation=int(os.getenv('MIN_FEEDBACKS_FOR_REPUTATION', '3'))
        )
    ...
    def validate(self) -> bool:
        ...
        # Validate ports
        ports = [self.borglife_mcp_port, self.borglife_agent_port, self.borglife_ui_port]
        ...
        for port_name, port in [
            ...
            if not (1024 <= port <= 65535):
                errors.append(f"{port_name} port {port} is not in valid range (1024-65535)")
