...
class ProductionMetricsCollector:
    ...
    def __init__(self, metrics_backend: str = 'jsonl',
    ...
    async def record_demo_run(self, success: bool, duration: float,
                             cost: Decimal, steps_data: Dict[str, Any] = None) -> bool:
        ...
        try:
            ...
            successful_runs = int(total_runs * (self.metrics['demo_success_rate'] / 100)) if total_runs > 1 else 0
            if success:
                successful_runs += 1
            ...
            self.metrics['average_execution_time'] = ((current_avg * (total_runs - 1)) + duration) / total_runs
            ...
            print(f"📊 Metrics recorded: Run {total_runs}, Success: {success}, "
    ...
    def _record_step_metrics(self, steps_data: Dict[str, Any]):
        ...
        for step_name, step_info in steps_data.items():
            ...
            if len(self.metrics['step_execution_times'][step_name]) > 100:
                self.metrics['step_execution_times'][step_name] = self.metrics['step_execution_times'][step_name][-100:]
    ...
    def record_network_performance(self, endpoint: str, response_time: float,
                                  success: bool):
        ...
        perf['average_response_time'] = ((current_avg * (total_requests - 1)) + response_time) / total_requests
    ...
    def record_resource_usage(self, cpu_percent: float = None,
                             memory_mb: float = None, disk_usage: float = None):
    ...
    def generate_alerts(self) -> List[Dict[str, Any]]:
        ...
        if total_runs > 0:
            error_rate = sum(self.metrics.get('error_rate_by_type', {}).values()) / total_runs * 100
            if error_rate > 20:
                alerts.append({
                    'type': 'error_rate',
                    'severity': 'high',
    ...
    def _export_prometheus_format(self) -> str:
        ...
        lines.append(f'# HELP borglife_demo_avg_execution_time Average execution time in seconds')
        lines.append(f'# TYPE borglife_demo_avg_execution_time gauge')
        lines.append(f'borglife_demo_avg_execution_time {self.metrics["average_execution_time"]}')
