...
class DemoInputValidator:
    ...
    MALICIOUS_PATTERNS = [
        ...
        r'confirm\s*\(', r'console\.', r'process\.', r'os\.',
        r'subprocess', r'import\s+os', r'import\s+subprocess',
        r'exec\s*\(', r'system\s*\(', r'popen\s*\('
    ]

    # Valid DNA structure patterns
    VALID_DNA_KEYS = {
        'borg_id', 'version', 'cells', 'organs', 'phenotype',
    ...
    def validate_task_description(self, task: str) -> Dict[str, Any]:
        ...
        for keyword in harmful_keywords:
            if keyword.lower() in task.lower():
                result['warnings'].append(f'Potentially harmful keyword detected: {keyword.strip()}')
    ...
    def validate_cost_limit(self, cost: Any, max_limit: Decimal = Decimal('1.0')) -> Dict[str, Any]:
    ...
    def validate_address(self, address: str, network: str = 'westend') -> Dict[str, Any]:
        ...
        if network == 'westend':
            ...
            if not address.startswith(('C', 'D', 'F', 'G', 'H', 'J')):  # Kusama prefixes
                result['valid'] = False
