...
class InterBorgTransfer:
        ...
        try:
            ...
            if not from_keypair:
                transfer_result["errors"].append(
                    f"No keypair found in macOS Keychain for borg {from_borg_id}"
                )
                return transfer_result
            ...
            if sender_balance < amount_planck:
                transfer_result["errors"].append(
                    f"Insufficient balance: {sender_balance} < {amount_planck} planck"
                )
                return transfer_result
            ...
            if not transfer_tx.get("success"):
                transfer_result["errors"].append(
                    f"Blockchain transfer failed: {transfer_tx.get('error', 'Unknown error')}"
                )
                return transfer_result
    ...
    async def _update_balances_wnd(
        self, from_borg_id: str, to_borg_id: str, amount_wnd: float
    ) -> None:
    ...
    async def get_transfer_history(
        ...
        try:
            ...
            if self.address_manager.supabase:
                result = (
                    self.address_manager.supabase.table("transfer_transactions")
                    .select("*")
                    .or_(f"from_borg_id.eq.{borg_id},to_borg_id.eq.{borg_id}")
                ...
                for record in result.data or []:
                    ...
                    amount_usdb = Decimal(str(record["amount_wei"])) / Decimal(
                        "1000000000000"
                    )
                    ...
                    transfers.append(
                        {
                            ...
                            "direction": (
                                "sent"
                                if record["from_borg_id"] == borg_id
                                else "received"
                            ),
                        }
                    )
    ...
    async def validate_transfer_request(
        self, from_borg_id: str, to_borg_id: str, amount: Decimal
    ) -> Dict[str, Any]:
        """
    ...
    async def get_borg_balance_summary(self, borg_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            asset_id = self.westend_adapter._get_usdb_asset_id()
            blockchain_balances = await self.westend_adapter.get_dual_balance(
                address, asset_id
            )
            ...
            wnd_balance_tokens = Decimal(
                str(blockchain_balances.get("wnd", 0))
            ) / Decimal("1000000000000")
            usdb_balance_tokens = Decimal(
                str(blockchain_balances.get("usdb", 0))
            ) / Decimal("1000000000000")
            ...
            return {
                ...
                "balances": {
                    "WND": {
                        ...
                        "database": str(
                            Decimal(str(wnd_balance_db)) / Decimal("1000000000000")
                        ),
                    },
                    "USDB": {
                        "blockchain": str(usdb_balance_tokens),
                        "database": str(
                            Decimal(str(usdb_balance_db)) / Decimal("1000000000000")
                        ),
                    },
                },
