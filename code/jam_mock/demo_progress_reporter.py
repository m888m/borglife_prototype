...
class DemoProgressReporter:
    ...
    def __init__(
        self, show_progress: bool = True, progress_callback: Optional[Callable] = None
    ):
    ...
    def complete_step(self, step_name: str, success: bool = True):
        ...
        if step_name in self.step_progress:
            ...
            duration = (
                self.step_progress[step_name]["end_time"]
                - self.step_progress[step_name]["start_time"]
            )

            status_icon = "✅" if success else "❌"
            self._report_progress(
                step_name, 100, f"{status_icon} Completed in {duration:.1f}s"
            )
    ...
    def get_overall_progress(self) -> Dict[str, Any]:
        ...
        completed_steps = sum(
            1 for step in self.step_progress.values() if step["status"] == "completed"
        )
    ...
    def show_step_summary(self):
        ...
        print(
            f"📈 Success Rate: {overall['completed_steps']}/{overall['total_steps']} steps"
        )
...
class BorgLifeDemoProgress:
    ...
    def update_step(self, step_key: str, progress: float, message: str = ""):
        ...
        completed_weight = sum(
            ...
            if key != step_key
            and self.reporter.get_step_status(info["name"])
    ...
    def _show_overall_progress(self, progress: float):
        ...
        if (
            abs(progress - self.current_overall_progress) > 1
        ):  # Only update if significant change
            self.current_overall_progress = progress
            self.reporter._report_progress(
                "overall", progress, f"Overall Progress: {progress:.1f}%"
            )
