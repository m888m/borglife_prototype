...
class DemoAuditLogger:
    ...
    def log_event(
        self, operation: str, message: str, details: Dict[str, Any] = None
    ) -> bool:
    ...
    def log_transaction_operation(
        ...
        details = {"tx_hash": tx_hash, "amount": amount, "operation_type": operation}
    ...
    def log_security_event(
        ...
        security_details = {"event_type": event_type, "severity": severity, **details}
        status = (
            "error"
            if severity in ["high", "critical"]
            else "warning" if severity == "medium" else "success"
        )
    ...
    def generate_compliance_report(
        ...
        for entry in entries:
            ...
            report["operations_by_status"][status] = (
                report["operations_by_status"].get(status, 0) + 1
            )
    ...
    def cleanup_old_logs(self, days_to_keep: int = 90) -> int:
        ...
        try:
            ...
            removed_count = (
                len(temp_entries) - len(temp_entries)
                if "temp_entries" in locals()
                else 0
            )
