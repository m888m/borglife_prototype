...
class OrganFallbackManager:
    ...
    FALLBACK_MAPPING = {
        "gmail": [
            {
                "level": FallbackLevel.ARCHON,
                "tool": "archon:send_email",
                "description": "Archon email service",
            },
            {
                "level": FallbackLevel.CACHED,
                "description": "Skip email operations (cached)",
            },
        ],
        "stripe": [
            {
                "level": FallbackLevel.ERROR,
                "description": "No payment fallback available",
            }
        ],
        "bitcoin": [
            {
                "level": FallbackLevel.ARCHON,
                "tool": "archon:blockchain_query",
                "description": "Archon blockchain service",
            },
            {
                "level": FallbackLevel.CACHED,
                "description": "Use cached blockchain data",
            },
        ],
        "mongodb": [
            {
            ...
            {
                "level": FallbackLevel.CACHED,
                "description": "Use cached database results",
            },
        ],
        "duckduckgo": [
            {
        ...
        "grafana": [
            {
                "level": FallbackLevel.SECONDARY,
                "organ": "prometheus",
                "description": "Prometheus metrics fallback",
            },
        ...
        "wikipedia": [
            {
                "level": FallbackLevel.SECONDARY,
                "organ": "wolfram",
                "description": "Wolfram knowledge fallback",
            },
        ...
        "arxiv": [
            {
                "level": FallbackLevel.SECONDARY,
                "organ": "semantic_scholar",
                "description": "Semantic Scholar fallback",
            },
    ...
    def __init__(self, archon_adapter, cache_manager=None):
        ...
        self.cache_manager = cache_manager or getattr(
            archon_adapter, "cache_manager", None
        )
            ...
            if fallback["level"] == FallbackLevel.SECONDARY:
                ...
                if archon_tool:
                    try:
                        ...
                        archon_params = self._convert_params_for_archon(
                            params, primary_organ, archon_tool
                        )
                ...
                if self.cache_manager:
                    try:
                        cache_key = self._generate_cache_key(
                            primary_organ, tool, params
                        )
                        result = await self.cache_manager.get_cached_result(cache_key)
                        if result:
                            self._record_fallback_usage(
                                primary_organ, fallback["level"]
                            )
                            return (
                                result,
                                fallback["level"],
                                f"{fallback['description']} (stale data)",
                            )
                    except Exception as e:
    ...
    def _convert_params_for_archon(
        self, original_params: Dict[str, Any], original_organ: str, archon_tool: str
    ) -> Dict[str, Any]:
        """
    ...
    def _generate_cache_key(self, organ: str, tool: str, params: Dict[str, Any]) -> str:
        """Generate cache key for fallback results"""
        # Create deterministic key from organ, tool, and key params
        key_params = {
            k: v for k, v in params.items() if k in ["query", "id", "hash", "address"]
        }
