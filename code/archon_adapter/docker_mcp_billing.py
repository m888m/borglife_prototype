...
class DockerMCPBilling:
    ...
    async def track_organ_usage(
        ...
        logger.info(
            f"Tracked usage: {borg_id} used {organ_name}:{operation} for {final_cost} DOT"
        )
        return final_cost
    ...
    async def deduct_from_borg_wealth(
        ...
        try:
            ...
            if current_wealth < cost:
                logger.warning(
                    f"Insufficient funds for borg {borg_id}: {current_wealth} < {cost}"
                )
                return False
            ...
            logger.info(
                f"Deducted {cost} DOT from borg {borg_id} wealth: {current_wealth} → {new_wealth}"
            )
            return True
    ...
    async def get_borg_usage_summary(
        ...
        """
        if not self.supabase:
            return {"total_cost": Decimal("0"), "organ_breakdown": {}, "usage_count": 0}
    ...
    def _calculate_size_multiplier(self, params: Dict[str, Any]) -> Decimal:
        ...
        if any(
            keyword in str(params).lower() for keyword in ["list", "search", "query"]
        ):
