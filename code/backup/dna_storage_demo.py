...
from jam_mock.demo_metrics import DemoAlertManager, DemoMetricsCollector
...
class BorgLifeDNADemo:
    ...
    def __init__(self):
        ...
        self.westend_adapter = WestendAdapter(
            "wss://westend-rpc.polkadot.io", connect_immediately=True
        )
        self.transaction_manager = TransactionManager(
            self.westend_adapter, self.keypair_manager
        )
    ...
    async def run_complete_demo(self) -> Dict[str, Any]:
        ...
        results = {"steps_completed": [], "errors": [], "metrics": {}, "success": False}
    ...
    async def _step_4_submit_transaction(self, results: Dict[str, Any]):
        ...
        try:
            ...
            demo_keypair = self.keypair_manager.create_keypair_from_seed(
                keypair_name, seed_hex, save_to_disk=False
            )
            ...
            if tx_result.get("success"):
                ...
                print(
                    f"   🔗 Transaction hash: {tx_result.get('transaction_hash', 'N/A')[:20]}..."
                )
                print(f"   💵 Fee paid: {tx_result.get('fee_paid', 'N/A')} WND")

                results["steps_completed"].append("transaction_submission")
                results["metrics"]["transaction_hash"] = tx_result.get(
                    "transaction_hash"
                )
                ...
                raise Exception(
                    f"Transaction failed: {tx_result.get('error', 'Unknown error')}"
                )
    ...
    async def _step_5_confirm_transaction(self, results: Dict[str, Any]):
        ...
        try:
            ...
            raise Exception(
                f"Transaction confirmation timeout after {max_wait_time} seconds"
            )
    ...
    async def _step_6_verify_integrity(self, results: Dict[str, Any]):
        ...
        try:
            ...
            if retrieved_hash:
                ...
                print(
                    f"   🔍 Hash integrity: {'✅ VERIFIED' if hash_matches else '❌ MISMATCH'}"
                )
                ...
                if hash_matches:
                    ...
                    results["errors"].append(
                        "DNA hash mismatch - integrity verification failed"
                    )
            else:
            ...
            print(
                f"   🔄 Round-trip integrity: {'✅ VERIFIED' if integrity_ok else '❌ FAILED'}"
            )
        ...
        if self.metrics.current_run:
            ...
            if results["success"]:
                ...
                alerts = self.alerts.check_all_alerts(
                    current_balance=balance, current_duration=run_metrics.total_duration
                )
                if alerts:
                    print("\n⚠️  ALERTS:")
                    for alert in alerts:
        ...
        if results["metrics"]:
            ...
            for key, value in results["metrics"].items():
                if isinstance(value, float):
                    ...
                    print(
                        f"   • {key.replace('_', ' ').title()}: N/A (Phase 1 limitation)"
                    )
                else:
...
async def main():
    ...
    try:
        ...
        if results["success"]:
            ...
            print(
                "BorgLife Phase 1 DNA storage is fully operational on Westend testnet!"
            )
            return 0
        else:
