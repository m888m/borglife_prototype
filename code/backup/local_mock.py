...
class LocalJAMMock(JAMInterface):
    ...
    def __init__(
        self, persistence_file: Optional[str] = None, mode: JAMMode = JAMMode.MOCK
    ):
        """
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_transactions": sum(
                len(txns) for txns in self.transaction_history.values()
            ),
    ...
    async def retrieve_dna_hash_from_blockchain(
        self, borg_id: str, block_range: Optional[Tuple[int, int]] = None
    ...
    async def verify_dna_integrity_with_proof(
        ...
        return {
            ...
            "block_number": (
                blockchain_data["block_number"] if blockchain_data else None
            ),
            "transaction_hash": (
                blockchain_data["transaction_hash"] if blockchain_data else None
            ),
            "proof_data": (
                {
    ...
    async def transfer_from_dispenser(
        self, from_address: str, to_borg_id: str, amount: Decimal, dispenser_keypair
    ) -> Dict[str, Any]:
        """
        Mock transfer from dispenser to borg.
        """
        success = await self.update_wealth(
            to_borg_id, amount, "funding", f"Dispenser transfer from {from_address}"
        )
        self.block_height += 1
        return {
            "success": success,
            "block": self.block_height,
            "transaction_hash": f"mock_disp_tx_{to_borg_id}_{self.block_height}",
    ...
    def _load_persistence(self):
        ...
        try:
            ...
            self.wealth_balances = {
                k: Decimal(str(v)) for k, v in data.get("wealth_balances", {}).items()
            }
