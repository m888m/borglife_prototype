...
class JAMInterfaceMock:
    ...
    async def update_wealth_balance(
        self, borg_id: str, amount: Decimal, operation: str
    ) -> bool:
        """Update wealth balance for a borg."""
        self._record_call(
            "update_wealth_balance", borg_id=borg_id, amount=amount, operation=operation
        )
    ...
    async def deduct_cost(self, borg_id: str, cost: Decimal, description: str) -> bool:
        ...
        return await self.update_wealth_balance(
            borg_id, -cost, f"cost_deduction: {description}"
        )

    async def add_reward(self, borg_id: str, reward: Decimal, description: str) -> bool:
        """Add reward to borg wealth."""
        return await self.update_wealth_balance(
            borg_id, reward, f"reward: {description}"
        )
    ...
    async def calculate_task_cost(
        ...
        self._record_call(
            "calculate_task_cost", task_type=task_type, complexity=complexity
        )
    ...
    async def estimate_organ_cost(
        self, organ_name: str, operation: str, params: Dict[str, Any]
    ) -> Decimal:
    ...
    async def get_transaction_history(
        ...
        borg_transactions = [
            t for t in self.transaction_history if t["borg_id"] == borg_id
        ]
        return borg_transactions[-limit:]  # Return most recent

    async def validate_economic_constraints(
        self, borg_id: str, operation_cost: Decimal
    ) -> Dict[str, Any]:
        """Validate if an operation can be performed economically."""
        self._record_call(
            "validate_economic_constraints",
            borg_id=borg_id,
        ...
        if self._should_fail():
            raise Exception(
                f"Mock JAM failure: validate_economic_constraints for {borg_id}"
            )
        ...
        return {
            ...
            "remaining_balance": (
                str(current_balance - operation_cost)
                if current_balance >= operation_cost
                else str(current_balance)
            ),
        }
    ...
    async def get_economic_metrics(self, borg_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_transaction": str(
                (total_earned - total_spent) / max(len(transactions), 1)
            ),
        }
