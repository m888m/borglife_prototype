...
from synthesis.dna_parser import DNAParser, BorgDNA
...
class BorgDesignerUI:
    ...
    def render_main_ui(self):
        ...
        st.set_page_config(
            page_title="BorgLife Designer",
            page_icon="🤖",
            layout="wide"
        )

        st.title("🤖 BorgLife Designer")
        st.markdown("Create and test autonomous borg phenotypes using Archon cells and Docker MCP organs")
        ...
        with col2:
            ...
            for organ_id, organ_info in self.docker_mcp_organs.items():
                with st.expander(f"{organ_info['name']} ({organ_id})"):
                    ...
                    health_color = "🟢" if organ_info['health_status'] == 'healthy' else "🔴"
                    st.write(f"**Health:** {health_color} {organ_info['health_status']}")
    ...
    def render_cost_analysis(self):
        ...
        total_cell_cost = sum(cell['cost_per_call'] for cell in self.current_phenotype.cells.values())
        total_organ_cost = sum(organ['cost_per_call'] for organ in self.current_phenotype.organs.values())
        ...
        with col3:
            st.metric("Total Estimated", f"{total_cell_cost + total_organ_cost:.6f} DOT/call")
    ...
    def render_performance_dashboard(self):
        ...
        with col1:
            st.metric("Avg Execution Time", f"{sum(execution_times)/len(execution_times):.2f}s")
    ...
    def add_component_to_phenotype(self, component_type: str, component_id: str, component_info: Dict[str, Any]):
        """Add a component to the current phenotype configuration."""
        if 'phenotype_config' not in st.session_state:
            st.session_state.phenotype_config = {'cells': {}, 'organs': {}, 'metadata': {}}
    ...
    async def build_phenotype_from_config(self):
        ...
        try:
            ...
            st.write(f"Built phenotype with {len(phenotype.cells)} cells and {len(phenotype.organs)} organs")
    ...
    def create_dna_from_config(self, config: Dict[str, Any]) -> BorgDNA:
        ...
        from synthesis.dna_parser import DNAHeader, Cell, Organ
        ...
        for organ_id, organ_info in config['organs'].items():
            organ = Organ(
                name=organ_id,
                mcp_tool=organ_info['tools'][0] if organ_info.get('tools') else f"{organ_id}:tool",
