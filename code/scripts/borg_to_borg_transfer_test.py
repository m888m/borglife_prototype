                ...
                if private_key_hex and public_key_hex and address == target_address:
                    ...
                    try:
                        ...
                        if reconstructed_address == target_address:
                            print(
                                f"   🎉 Successfully found and verified keypair for address {target_address}"
                            )
                            return keypair
                        else:
                            print(
                                f"     ❌ Address mismatch: expected {target_address}, got {reconstructed_address}"
                            )
                    except Exception as e:
                        print(f"     ❌ Keypair reconstruction failed: {e}")
                else:
                    print(
                        f"     ❌ Missing keypair components (private_key: {private_key_hex is not None}, public_key: {public_key_hex is not None})"
                    )
...
async def main():
    ...
    try:
        ...
        if not from_keypair:
            print(
                f"   Keypair not found for borg ID {from_borg_id}, searching by address..."
            )
            from_keypair = find_keypair_by_address(from_address)

        if not from_keypair:
            raise Exception(
                f"No keypair found in macOS Keychain for borg {from_borg_id} (address: {from_address})"
            )

        if from_keypair.ss58_address != from_address:
            raise Exception(
                f"Keypair address mismatch for borg {from_borg_id}: expected {from_address}, got {from_keypair.ss58_address}"
            )
        ...
        results["initial_balances"] = {
            from_borg_id: {
                ...
                "balance_wnd": from_balance_wnd,
                "balance_planck": from_balance,
            },
            to_borg_id: {
                "address": to_address,
                "balance_wnd": to_balance_wnd,
                "balance_planck": to_balance,
            },
        }
        ...
        if from_balance_wnd < transfer_amount_wnd:
            raise Exception(
                f"Insufficient balance: {from_balance_wnd} < {transfer_amount_wnd}"
            )
        ...
        transfer_result = {
            ...
            "to_borg_id": to_borg_id,
            "errors": (
                [transfer_tx.get("error", "Unknown error")]
        ...
        results["final_balances"] = {
            from_borg_id: {
                "address": from_address,
                "balance_wnd": final_from_balance_wnd,
                "balance_planck": final_from_balance,
            },
            to_borg_id: {
                "address": to_address,
                "balance_wnd": final_to_balance_wnd,
                "balance_planck": final_to_balance,
            },
        }
    ...
    results_file = (
        f"borg_to_borg_transfer_results_{int(datetime.utcnow().timestamp())}.json"
    )
    ...
    if results["success"]:
        ...
        print(
            "🌐 View on Westend Explorer: https://westend.subscan.io/extrinsic/"
            + results["transfer_result"]["transaction_hash"]
        )
    else:
