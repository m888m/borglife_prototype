...
async def main():
    ...
    dna_hash = (
        "b2c3d4e5f67890123456789012345678901234567890123456789012345678901"  # 64 chars
    )
    ...
    call = westend_adapter.substrate.compose_call(
        ...
        call_params={"dest": borg2_address, "value": int(transfer_amount_1 * (10**12))},
    )

    # Create signed extrinsic
    extrinsic = westend_adapter.substrate.create_signed_extrinsic(
        call=call, keypair=bt_keypair
    )
    ...
    bt_ok = (
        abs(bt_actual_change - bt_expected_change) < 0.01
    )  # Allow small fee differences
    ...
    if bt_ok and b2_ok:
        ...
        print(
            f"   Borg Tester net change: {bt_actual_change:.6f} WND (expected: {bt_expected_change:.6f})"
        )
        print(
            f"   Borg 2 net change: {b2_actual_change:.6f} WND (expected: {b2_expected_change:.6f})"
        )
    else:
        print("⚠️ TRANSFER VALIDATION WARNING")
        print("   Balance changes may include network fees")
        print(
            f"   Borg Tester: {bt_actual_change:.6f} WND (expected: {bt_expected_change:.6f})"
        )
        print(
            f"   Borg 2: {b2_actual_change:.6f} WND (expected: {b2_expected_change:.6f})"
        )
