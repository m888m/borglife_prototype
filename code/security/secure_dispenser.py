...
class SecureDispenser:
    ...
    def _ensure_keystore(self):
        ...
        if not os.path.exists(self.keystore_path):
            ...
            print(
                'Run: python3 -c "from code.security.secure_dispenser import SecureDispenser; d = SecureDispenser(); d.setup_keystore()"'
            )
            return False
        return True
    ...
    def setup_keystore(self, dispenser_seed: Optional[str] = None) -> bool:
        ...
        try:
            ...
            print(
                f"✅ Dispenser keystore created with macOS Keychain storage: {self.keystore_path}"
            )
    ...
    def unlock_for_session(self, session_duration_hours: int = 1) -> bool:
        ...
        try:
            ...
            if setup_version not in ["3.0", "4.0"]:
                raise ValueError(
                    f"Incompatible keystore version {setup_version} - please recreate keystore"
                )
    ...
    async def transfer_wnd_to_borg(
        self, borg_address: str, borg_id: str, amount_wnd: float
    ) -> Dict[str, Any]:
        """
    ...
    async def transfer_wnd_from_borg(
        self, borg_id: str, amount_wnd: float
    ) -> Dict[str, Any]:
        """
        ...
        result = {
            ...
            "from_address": None,
            "to_address": (
                self.unlocked_keypair.ss58_address if self.unlocked_keypair else None
            ),
            "block_number": None,
            "block_hash": None,
        }
        ...
        try:
            ...
            if not borg_keypair:
                result["error"] = (
                    f"Could not load keypair for borg {borg_id} from keyring"
                )
                return result
            ...
            borg_balance = await westend_adapter.get_wnd_balance(
                borg_keypair.ss58_address
            )
            if borg_balance < amount_planck:
                result["error"] = (
                    f"Insufficient borg balance: {borg_balance} < {amount_planck} planck"
                )
                return result
            ...
            self.audit_logger.log_event(
                ...
                {"borg_id": borg_id, "amount_requested": amount_wnd, "error": str(e)},
            )
            return result

    async def _update_transfer_metadata(
        self,
        from_borg_id: str,
        to_borg_id: str,
        currency: str,
        ...
        try:
            ...
            supabase.table("transfer_transactions").upsert(
                tx_record, on_conflict="tx_id"
            ).execute()
            ...
            if (
                from_borg_id != "dispenser"
            ):  # Don't update dispenser balance in borg_balances
                try:
                    current_from_balance = (
                        supabase.table("borg_balances")
                        .select("balance_wei")
                        .eq("borg_id", from_borg_id)
                        .eq("currency", currency)
                        .execute()
                    )
                    if current_from_balance.data:
                        new_from_balance = max(
                            0, current_from_balance.data[0]["balance_wei"] - amount_wei
                        )
                        supabase.table("borg_balances").update(
                            {"balance_wei": new_from_balance, "last_updated": "NOW()"}
                        ).eq("borg_id", from_borg_id).eq("currency", currency).execute()
                except Exception as e:
            ...
            try:
                ...
                if current_to_balance.data:
                    new_to_balance = (
                        current_to_balance.data[0]["balance_wei"] + amount_wei
                    )
                    supabase.table("borg_balances").update(
                        {"balance_wei": new_to_balance, "last_updated": "NOW()"}
                    ).eq("borg_id", to_borg_id).eq("currency", currency).execute()
                else:
    ...
    def transfer_wnd_fee(
        self, borg_address: str, borg_id: str, dna_hash: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if transfer_amount > self.max_transfer_amount:
                result["error"] = (
                    f"Transfer amount {transfer_amount} exceeds maximum {self.max_transfer_amount}"
                )
                return result
            ...
            if daily_used + transfer_amount > self.daily_limit:
                result["error"] = (
                    f"Daily limit exceeded: {daily_used + transfer_amount} > {self.daily_limit}"
                )
                return result
            ...
            self.audit_logger.log_event(
                ...
                {"borg_id": borg_id, "borg_address": borg_address, "error": str(e)},
            )
            return result
    ...
    async def mint_usdb_tokens(self, amount_usdb: float) -> Dict[str, Any]:
        ...
        try:
            ...
            extrinsic = self.asset_hub_adapter.substrate.create_signed_extrinsic(
                call=call, keypair=self.unlocked_keypair
            )

            # Submit and wait for inclusion
            receipt = self.asset_hub_adapter.substrate.submit_extrinsic(
    ...
    async def transfer_usdb_to_borg(
        self, borg_address: str, borg_id: str, amount_usdb: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if dispenser_balance < amount_planck:
                result["error"] = (
                    f"Insufficient dispenser balance: {dispenser_balance} < {amount_planck}"
                )
                return result
