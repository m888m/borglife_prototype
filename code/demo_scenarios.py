...
class DemoMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        success_rate = (
            (self.metrics["successful_runs"] / self.metrics["demo_runs"] * 100)
            if self.metrics["demo_runs"] > 0
            else 0
        )
        dna_integrity_rate = (
            (
        ...
        return {
            ...
            "avg_execution_time_seconds": (
                round(
                    self.metrics["total_execution_time"] / self.metrics["demo_runs"], 2
                )
                if self.metrics["demo_runs"] > 0
                else 0
            ),
            ...
            "archon_success_rate_percent": (
                round(
                    (self.metrics["archon_calls"] - self.metrics["archon_errors"])
                    / self.metrics["archon_calls"]
                    * 100,
                    2,
                )
                if self.metrics["archon_calls"] > 0
                else 0
            ),
        }
...
class BorgLifeDemo:
    ...
    async def run_single_demo(
        self, borg_id: str, funding_amount: float = 0.1
    ) -> Dict[str, Any]:
        """
...
async def main():
    ...
    parser.add_argument(
        "--mode", choices=["local", "westend", "hybrid"], default="local"
    )
    parser.add_argument("--count", type=int, default=5, help="Number of demo runs")
    parser.add_argument(
        "--stress-test", type=int, help="Run stress test with N concurrent borgs"
    )
    parser.add_argument(
        "--westend", action="store_true", help="Include Westend validation"
    )
